...
function ByStudyType({ data, dataset, title }: { data: MetricRow[]; dataset: string; title: string }) {
  ...
  function renderChart({ data, width, height }: { data: StudyTypeCount[]; width?: number; height: number }) {
    ...
    return Plot.plot({ width: width ?? 0, height, style: { fontSize: "13.5px" }, marginTop: 4, marginBottom: 4, marginLeft: 140, marginRight: 0, x: { axis: null }, y: { domain: data.map((item) => item.name), label: null, tickSize: 0, tickPadding: 8, tickFormat: (label) => label.replaceAll(/(gwas|qtl)/ig, m => m.toUpperCase()) }, marks: [
