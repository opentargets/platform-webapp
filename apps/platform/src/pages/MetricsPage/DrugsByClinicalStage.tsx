...
type ClinicalStageCount = { name: string; count: number };
...
function DrugsByClinicalStage({ data }: { data: MetricRow[] }) {
  ...
  const chartData: ClinicalStageCount[] = data
    ...
    .map((row) => ({
      name:
        clinicalStageCategories[row.group_value as keyof typeof clinicalStageCategories]?.label ??
        row.group_value,
      count: row.value,
    }))
